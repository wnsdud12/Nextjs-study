...
function LNBExpandableItem({
  label,
  path,
  children = [],
  depth = 0,
}: MenuItem & { depth?: number }) {
  ...
  return (
    <div className={`pl-${depth * 4}`}>
      <div>
        {path ? (
          <LNBLinkItem label={label} path={path} />
        ) : (
          <button
      ...
      {open && (
        <div>
          {children.map((child, idx) =>
            child.children ? (
              <LNBExpandableItem key={idx} {...child} depth={depth + 1} />
            ) : (
              <LNBLinkItem key={idx} label={child.label} path={child.path!} />
            )
          )}
        </div>
      )}
...
const LNB = () => {
  return (
    <nav>
    {LNBMenuList.map((item, idx) =>
      item.children ? (
        <LNBExpandableItem key={idx} {...item} />
      ) : (
        <LNBLinkItem key={idx} label={item.label} path={item.path!} />
      )
    )}
  </nav>
  );
